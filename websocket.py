...
async def relay_message(message, sessions):
...
async def chat(websocket, sessions={}):
    ...
    await relay_message(f"{websocket.remote_address} joined the chat!", sessions)
    try:
        async for message in websocket:
            await relay_message(f"from {websocket.remote_address}: {message}", sessions)
    finally:
        del sessions[remote]
        print(f"[INFO]: client {websocket.remote_address} disconnected")
        await relay_message(f"from {websocket.remote_address} left the chat!", sessions)
...
async def web_socket_router(websocket, path):
    ...
    if path == '/':
        ...
        print(f"[INFO]: client {websocket.remote_address} connected through /echo")
        await echo(websocket)
    elif path == '/chat':
        print(f"[INFO]: client {websocket.remote_address} connected through /chat")
